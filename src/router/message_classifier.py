...
def message_router(model, agent, query: str, config: dict):
    ...
    # Fora do escopo --> Dar uma resposta neutra
    elif categoria == "FORA_DO_ESCOPO":
        print("[ROTA] Fora do escopo")
        prompt_text = prompt_foraEscopo.format(user_input=query, memory="")
        response = model.invoke([HumanMessage(content=prompt_text)])

    # Dentro do escopo --> INFO(Caso o agente não saiba o que fazer)
    else:
